...
gc <- import_report(growth_curves) %>%
    ...
    filter(drug_used=="None", !series_id=="AH", gc_date > as.Date("2022-12-31"))
...
tgen_metric <- ggplot(gc, aes(x=genus_species, y=t_gen)) + 
    geom_boxplot(aes(fill=genus_species), 
                 outlier.colour = NA, 
                 width = 0.4, 
                 alpha = 0.8) +
    ...
    theme(axis.text.x = element_text(angle = 30, 
                                     ...
                                     vjust = 1)) +
    xlab("Species") +
    ylab("Doubling time, t_gen")

stack_plots <- k_metric/r_metric

ggsave(paste0("images/2023_growth_curves/",Sys.Date(),"_k_r_summaries.png"),
       stack_plots,
       device=png, 
       bg="white", 
       dpi=300, 
